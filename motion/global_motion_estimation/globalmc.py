...
def overlay_images(
    ...
    """
    Load two images, overlay using an offset and save the new image

    Params
      previous_path:  path to previous image (will be background)
      current_path:  path to current image
      x_offset:  offset in x axis
      y_offset:  offset in y axis
      show_box:  draw a box around the background image after overlay
    ...
    # Prepare affine matrix for overlay
    M = np.array([[1, 0, x_offset - min_x], [0, 1, y_offset - min_y]], dtype=np.float32)
    ...
    if show_box:
        # Draw box around background image for visualisation
        cv2.rectangle(
            expanded, (bg_x, bg_y), (bg_x + bw - 1, bg_y + bh - 1), (0, 255, 255), 1
        )

    # Save result
    cv2.imwrite(str(output_path), expanded)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="globalmc", description="Join images with given alignment"
    )
    ...
    parser.add_argument(
        ...
        help="Show box around previous image",
    )
    parser.add_argument("--output", type=Path, help="Output image")

    args = parser.parse_args()
    overlay_images(
        args.previous, args.current, args.xoff, args.yoff, args.outline, args.output
    )
