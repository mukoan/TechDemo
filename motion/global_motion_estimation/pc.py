...
def load_image_grey(path: Path) -> cv2.typing.MatLike:
    """
...
def plot_phase_correlation_surface(img1: cv2.typing.MatLike, img2: cv2.typing.MatLike):
    """
...
def main(image1_path: Path, image2_path: Path, show: bool):
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="pc", description="Phase correlation for image alignment"
    )

    parser.add_argument("--current", type=Path, help="Current image")
    parser.add_argument("--previous", type=Path, help="Target image")
    parser.add_argument(
        "--show", help="Show PC surface", action=argparse.BooleanOptionalAction
    )
