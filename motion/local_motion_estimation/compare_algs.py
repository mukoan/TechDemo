...
def load_csv(filename: str) -> Tuple[list[float], list[int]]:
    """
    Read CSV file
...
def main():
    ...
    plt.plot(
        range(2, 2 + len(psnr_pmvfast)), psnr_pmvfast, label="PMVFAST", color="red"
    )
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(
        range(2, 2 + len(time_taken_2dfs)), time_taken_2dfs, label="2DFS", color="blue"
    )
    plt.plot(
        range(2, 2 + len(time_taken_pmvfast)),
        time_taken_pmvfast,
        label="PMVFAST",
        color="red",
    )
    ...
    print(
        f"2DFS: Average PSNR = {np.mean(psnr_np_2dfs):.2f} dB, Average Time = {np.mean(time_np_2dfs):.2f} us"
    )
    print(
        f"PMVFAST: Average PSNR = {np.mean(psnr_np_pmvfast):.2f} dB, Average Time = {np.mean(time_np_pmvfast):.2f} us"
    )
