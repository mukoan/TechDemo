...
def detect_and_match(
    ...
    kpc0, kpc1 = (
        viz2d.cm_prune(matches01["prune0"]),
        viz2d.cm_prune(matches01["prune1"]),
    )
    ...
    if save_single_keypoints:
        ...
        if keypoints_img is None:
            print(f"Could not read {keypoints_filename}")
        else:
...
def main(images_path: Path, output_path: Path):
    """
    ...
    while True:
        ...
        if not os.path.exists(current_image_path):
            raise FileNotFoundError(f"Could not read {current_image_path}")

        if not os.path.exists(previous_image_path):
            raise FileNotFoundError(f"Could not read {previous_image_path}")

        keypoints_output_path = os.path.join(
            output_path, f"keypoints_{current_index:05d}.png"
        )
        matches_output_path = os.path.join(
            output_path, f"matches_{current_index:05d}.png"
        )
