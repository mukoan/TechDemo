...
def process(video_path: Path, images_path: Path, output_path: Path, method: str):
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="workfeatures.py", description="Evaluate feature detection and matching"
    )

    parser.add_argument("--video", type=Path, help="Input video")
    parser.add_argument(
        "--images_dir", type=Path, help="Directory to store extracted images"
    )
    parser.add_argument(
        "--output_dir",
        type=Path,
        help="Directory to store output keypont and matches images",
    )
    parser.add_argument(
        "--method", type=str, default="sift", help="Feature detection method"
    )
