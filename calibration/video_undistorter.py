...
def undistort_video(
    input_file: Path, calibration_file: Path, mode: str, output_file: Path
):
    """
    ...
    new_camera_mtx, roi = cv2.getOptimalNewCameraMatrix(
        cam_mtx, dist, (width, height), 0, (width, height)
    )
    x, y, w_crop, h_crop = roi

    # Build FFmpeg filter string
    filter_chain = (
        f"lenscorrection=cx={cx}:cy={cy}:k1={k1}:k2={k2},crop={w_crop}:{h_crop}:{x}:{y}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--calibration", type=Path, help="Path to calibration file")
    parser.add_argument("--output", type=Path, help="Path for output video file")
    parser.add_argument(
        "--mode",
        choices=["pad", "scale"],
        default="pad",
