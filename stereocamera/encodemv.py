...
def load_intrinsics(yaml_path: Path) -> Tuple[cv2.typing.MatLike, cv2.typing.MatLike]:
    """
...
def load_extrinsics(yaml_path: Path) -> Tuple[cv2.typing.MatLike, cv2.typing.MatLike]:
    """
...
def make_streams(
    input_dir: Path,
    left_intrinsics: Path,
    right_intrinsics: Path,
    extrinsics: Path,
    scale: float,
):
    """
    ...
    R1, R2, P1, P2, Q = cv2.fisheye.stereoRectify(
        K1, D1, K2, D2, img_size, R, T, 0, fov_scale=scale
    )
    map11, map12 = cv2.fisheye.initUndistortRectifyMap(
        K1, D1, R1, P1, img_size, cv2.CV_16SC2
    )
    map21, map22 = cv2.fisheye.initUndistortRectifyMap(
    ...
    # Find all images
    frames_list = []
    for left_image in sorted((input_dir / "left").glob("*.png")) + sorted(
        (input_dir / "left").glob("*.jpg")
    ):
...
def encode_video(output_file: str):
    """
    ...
    cmd_fix = [
        bin_mp4box,
        "-add",
        f"mvhevc_output.hevc:fps={fps}:colr=nclx,bt709,bt709,bt709,off",
        "-new",
        str(output_file),
    ]
...
def mux_audio(original_audio: Path, offset: float, output_filename: Path):
    """
...
def process(args):
    ...
    make_streams(args.input, args.leftcal, args.rightcal, args.extcal, args.scale)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="encodemv", description="Encode MV-HEVC")
    ...
    parser.add_argument(
        "--offset", type=float, default=0.0, help="Offset start of left file (seconds)"
    )
    parser.add_argument(
        "--scale", type=float, default=0.95, help="Scale to remove border"
    )
