...
def process_images(input_dir: Path, lut_path: Path, output_dir: Path):
    """
    Process images
    ...
    if output_dir == input_dir:
        raise ValueError(
            f"Output directory cannot be same as input directory {str(input_dir)}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="correct_images", description="Apply a 3D LUT to a directory of images"
    )
