...
def setup_output(output_dir: Path):
    """
...
def extract_audio(file_path: Path) -> np.ndarray:
    """
...
def plot_correlation(raw_left: np.ndarray, raw_right: np.ndarray):
    """
...
def find_offset(raw_left: np.ndarray, raw_right: np.ndarray) -> Tuple[float, float]:
    """
...
def audio_sync(
    left: Path, right: Path, trimleft: int, trimright: int, lut: Path, output: Path
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--left", type=Path, help="Path to left video file", required=True
    )
    parser.add_argument(
        "--right", type=Path, help="Path to right video file, required=True"
    )
    parser.add_argument(
        "--output", type=Path, help="Path for output frames, required=True"
    )
    parser.add_argument(
        "--trimleft",
        type=int,
        default=2,
        help="Trim amount if left recording started first",
    )
    parser.add_argument(
        "--trimright",
        type=int,
        default=4,
        help="Trim amount if right recording started first",
    )
    parser.add_argument(
        "--lut", type=Path, help="Look Up Table for colour correction of right camera"
    )

    args = parser.parse_args()

    audio_sync(
        args.left, args.right, args.trimleft, args.trimright, args.lut, args.output
    )
