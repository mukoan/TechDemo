...
def make_lut(input_dir: Path, output_file: Path):
    """
    Build a LUT
    ...
    frames_list = []
    for left_image_filename in sorted((input_dir / "left").glob("*.png")) + sorted(
        (input_dir / "left").glob("*.jpg")
    ):
        right_image_filename = input_dir / "right" / left_image_filename.name
        if right_image_filename.exists():
            frames_list.append(left_image_filename.name)
        else:
            LOG.warning(
                f"Right image {right_image_filename} does not exist for left image {left_image_filename}"
            )
    ...
    for frame in frames_list:
        ...
        for match in flann_matches:
            ...
            if (
                np.all(left_rgb > 0.05)
                and np.all(left_rgb < 0.95)
                and np.all(right_rgb > 0.05)
                and np.all(right_rgb < 0.95)
            ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="generate_lut", description="Build a 3D LUT from stereo video frames"
    )
