...
def make_image_lists(project_dir: Path):
    """
...
def load_calibration(yaml_path: Path) -> Tuple[cv2.typing.MatLike, cv2.typing.MatLike]:
    """
...
def extract_features(project_dir: Path, left_cal_path: Path, right_cal_path: Path):
    """
...
def configure_rig(project_dir: Path):
    """
    ...
    cmd = [
        ...
        "rig_configurator",
        "--database_path",
        str(project_dir / "project.db"),
        "--rig_config_path",
        str(project_dir / "rig_config.json"),
    ]
    subprocess.run(cmd, check=True)


def run_matching(project_dir: Path):
    """
...
def sparse_reconstruction(project_dir: Path):
    """
    ...
    cmd_convertor = [
        ...
        "model_converter",
        "--input_path",
        str(project_dir / "sparse/0"),
        "--output_path",
        str(project_dir / "sparse.ply"),
        "--output_type",
        "PLY",
    ]
...
def run_sfm(project_dir: Path, left_cal_path: Path, right_cal_path: Path):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--project", type=Path, help="Path to project dir", required=True
    )
    parser.add_argument(
        "--leftcal", type=Path, help="Path to left intrinsics file", required=True
    )
    parser.add_argument(
        "--rightcal", type=Path, help="Path to right intrinsics file", required=True
    )
